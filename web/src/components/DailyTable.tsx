...
export default function DailyTable({ days }: { days: DailyRow[] }) {
  ...
  const productive = days.filter((r) => r.received_krx > 0);
  const avgKrx =
    productive.length > 0
      ? productive.reduce((a, r) => a + r.received_krx, 0) / productive.length
      : 0;
  const hasAny = productive.length > 0;
      ...
      <table className="table">
        ...
        <tbody>
          {!hasAny ? (
            ...
            [...days].reverse().map((r) => (
              <tr key={r.day} className={r.received_krx <= 0 ? 'row-zero' : undefined}>
                <td>{r.day}</td>
        ...
        {hasAny && (
          <tfoot>
            <tr>
              ...
              <td className="num">{productive.length}</td>
              <td className="num">—</td>
              <td className="num">{fmtUsdt(totalUsdt)}</td>
            </tr>
            <tr className="table-avg">
              <td>{t('daily.avg')}</td>
              <td className="num">{fmtKrx(avgKrx)}</td>
              <td className="num">—</td>
              <td className="num">—</td>
              <td className="num">—</td>
            </tr>
